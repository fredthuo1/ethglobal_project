...
describe('HyperlaneIsmFactory', async () => {
    ...
    it('deploys a simple ism', async () => {
        ...
        const matches = await (0, HyperlaneIsmFactory_1.moduleMatchesConfig)(chain, ism.address, config, factory.multiProvider, factory.getContracts(chain));
    ...
    for (let i = 0; i < 16; i++) {
        it('deploys a random ism config', async () => {
            ...
            try {
                const matches = await (0, HyperlaneIsmFactory_1.moduleMatchesConfig)(chain, ismAddress, config, factory.multiProvider, factory.getContracts(chain));
                (0, chai_1.expect)(matches).to.be.true;
            }
            catch (e) {
